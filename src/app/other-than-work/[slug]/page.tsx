...
export default function OtherThanWorkPostPage({
  params,
}: {
  params: { slug: string };
}) {
  const post = otherThanWorkPosts.find((p) => p.slug === params.slug);
    ...
    <article className="space-y-8">
      ...
      <div className="prose prose-invert mx-auto max-w-none lg:prose-lg">
        ...
        <p>
          ...
          application, you would fetch the full content for the post here,
...
export async function generateStaticParams() {
  return otherThanWorkPosts.map((post) => ({
    slug: post.slug,
  }));
}
